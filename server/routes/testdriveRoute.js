...
router.put("/done", (req, res) => {
    ...
    db.query("update testdrive set status = 'd' where tid = ?; delete from testdrive where tid = ?;", [id, id], (err, result) => {
        //db.query("update testdrive set status = 'd' where tid = ?; ", [id], (err, result) => {
        if (err) {
            console.log(err);
            res.send({ status: "false" });
        }
...
router.get("/viewPending", (req, res) => {
    db.query("select  tid, td_date, td_slot, vehid from testdrive where status = 'p'", (err, result) => {
        //db.query("select * from testdrive where status = 'p' ", (err, result) => {
        if (err) {
            console.log(err);
            res.send({ status: "false" });
        }
